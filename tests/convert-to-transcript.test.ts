...
describe("convert-to-transcript", () => {
	...
	it("handles text with no quotes (all Storyteller)", async () => {
		...
		try {
			const { exitCode } = await runScript([
				"--source",
				"data/audio/no-quote-source.txt",
				"--output",
				"data/audio/no-quote-transcript.txt",
			]);
			expect(exitCode).toBe(0);
	...
	it("handles text that starts with a quote", async () => {
		...
		try {
			const { exitCode } = await runScript([
				"--source",
				"data/audio/start-quote-source.txt",
				"--output",
				"data/audio/start-quote-transcript.txt",
			]);
			expect(exitCode).toBe(0);
	...
	it("handles text with unbalanced quotes", async () => {
		...
		try {
			const { exitCode } = await runScript([
				"--source",
				"data/audio/unbalanced-source.txt",
				"--output",
				"data/audio/unbalanced-transcript.txt",
			]);
			expect(exitCode).toBe(0);
	...
	it("fails on whitespace-only source file", async () => {
		...
		try {
			const { exitCode, stderr } = await runScript([
				"--source",
				"data/audio/whitespace-source.txt",
				"--output",
				"data/audio/test-out.txt",
			]);
