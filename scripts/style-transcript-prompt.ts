 ...
 * before sending it to Gemini TTS:
 *
 ...
 *
 * 7. **Preamble present** — There must be a short TTS instruction block
 *    before the transcript (e.g. "Make Storyteller sound…"). This prevents
 *    Gemini TTS from reading performance directions aloud.
 *
...
export function buildStylePrompt({
	...
	const system = [
		...
		"5. Keep exactly two speakers: Storyteller and Pixel. Do not rename, merge, or add speakers.",
		...
		"",
		"OUTPUT FORMAT:",
		"The first line MUST be a short TTS preamble telling the TTS model how each speaker should sound. Then a blank line, then the transcript with audio tags.",
		"",
		...
		"```",
		"",
		"IMPORTANT: The TTS preamble (first line) is the ONLY instruction the TTS model will receive. The transcript lines below it will be spoken aloud. Never put instructions, scene directions, or notes in the transcript section — only speaker-labelled lines with optional [audio tags].",
		"",
